...
const BookCard = ({ book, setBooks }) => {
  ...
  const isActive = location.pathName === `/book/${book.id}`;
  ...
  return (
    <>
      {/* CARD */}
      <Link to={`/book/${book._id}`} className={`relative block rounded-xl bg-baser-100 p-4 border transition-all duration-200 ${isActive ? "border-primary shadow-lg" : "border-base-300"} hover:border-primary hover:shadow-xl`}>
        {/* Top Row */}
      ...
      {showModal && (<dialog className="modal modal-open">
        <div className="modal-box">
          <h3 className="foot-bold text-lg text-error flex items-center gap-2">
            <Trash2 className="size-5" /> Delete Book </h3>
          <p className="py-4 text-base-content/70">
            Arev you sure you want to delete
